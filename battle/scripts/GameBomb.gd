...
func show_dots():
	for dot in dots:
		var cell = $Panel/Grid.get_node("Celula_%d" % dot)
...
func add_to_trail(index):
	trail.append(index)
	var cell = $Panel/Grid.get_node("Celula_%d" % index)
	if dots.has(index):
		cell.modulate = Color(0, 0.5, 1)
...
func reset_trail():
	for i in range(TOTAL_CELLS):
		if not dots.has(i):
			var cell = $Panel/Grid.get_node("Celula_%d" % i)
			cell.modulate = Color(1, 1, 1)
	dragging = false
	start_dot = null
	trail.clear()
		...
		if not trail.has(index) and is_adjacent(trail[-1], index):
			...
			if dots.has(index) and index != start_dot:
				...
				if has_bomb:
					...
					emit_signal("correct_answer_hit", int(25))#int(10 + 20 * State.save_data["memory"] * 0.05)) 
					emit_signal("game_finished", false) 
				reset_trail()
